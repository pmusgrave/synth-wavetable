...
uint8 Codec_SendData(uint8 regAddr, uint16 data)
{
	...
	temp = CodecI2CM_I2CMasterSendStart(CODEC_I2C_ADDR, I2C_WRITE_OPERATION, I2C_TIMEOUT_MS); 
	if(temp == CodecI2CM_I2C_MSTR_NO_ERROR)
	{
		temp = CodecI2CM_I2CMasterWriteByte(((regAddr << 0x01) | (HI8(data) & 0x01)), I2C_TIMEOUT_MS);
		
		if(temp == CodecI2CM_I2C_MSTR_NO_ERROR)
		{
			temp = CodecI2CM_I2CMasterWriteByte(LO8(data), I2C_TIMEOUT_MS);
			
			if(temp == CodecI2CM_I2C_MSTR_NO_ERROR)
			{
				temp = CodecI2CM_I2CMasterSendStop(I2C_TIMEOUT_MS);
			}
		}
	}
