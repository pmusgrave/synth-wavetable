...
static USBFS_1_BACKUP_STRUCT  USBFS_1_backup;
    ...
    *******************************************************************************/
    ...
    {
    #ifdef USBFS_1_DP_ISR_ENTRY_CALLBACK
	...
	#ifdef USBFS_1_DP_ISR_EXIT_CALLBACK
...
void USBFS_1_Resume(void) 
{
    ...
    if (0u != USBFS_1_backup.enableState)
    {
    #if (USBFS_1_DP_ISR_ACTIVE)
        ...
        USBFS_1_INTR_SIE_MASK_REG = (uint32) USBFS_1_backup.intrSeiMask;
        ...
        CyDelayUs(USBFS_1_WAIT_VREF_STABILITY);
        ...
        /* Restore state of USB regulator and wait until it supples stable power. */
        USBFS_1_CR1_REG |= USBFS_1_backup.mode;
        CyDelayUs(USBFS_1_WAIT_VREF_STABILITY);   /*~50ns delay. */
