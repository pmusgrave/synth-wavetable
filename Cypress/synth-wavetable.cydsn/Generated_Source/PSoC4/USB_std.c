...
uint8 USB_HandleStandardRqst(void) 
{
    ...
    uint32 bmRequestType = USB_bmRequestTypeReg;
...
void USB_ConfigReg(void) 
{
    ...
    USB_ARB_CFG_REG |= (uint32)  USB_ARB_CFG_CFG_CMP;
    ...
    USB_SIE_EP_INT_EN_REG = (uint32) USB_DEFAULT_SIE_EP_INT_EN;
}
    ...
    if (USB_configuration > 0u)
    {
        ...
        */
        ...
        for (i = 0u; i < ep; i++)
        {
            ...
            {
                curEp  = ((uint32) pEP->addr & USB_DIR_UNUSED);
                epType = ((uint32) pEP->attributes & USB_EP_TYPE_MASK);
            ...
            #if (USB_EP_MANAGEMENT_DMA_AUTO)
                /* Clear IN data ready. */
                USB_ARB_EP_BASE.arbEp[curEp].epCfg &= (uint32) ~USB_ARB_EPX_CFG_IN_DATA_RDY;

                /* Select endpoint number of reconfiguration */
                USB_DYN_RECONFIG_REG = (uint32) ((curEp - 1u) << USB_DYN_RECONFIG_EP_SHIFT);
                ...
                if (0u != (pEP->addr & USB_DIR_IN))
                {
                    ...
                    USB_EP_TYPE_REG &= (uint32) ~(uint32)((uint32) 0x01u << (curEp - 1u));
                ...
                {
                    /* Set endpoint type: 0 - IN and 1- OUT. */
                    USB_EP_TYPE_REG |= (uint32) ((uint32) 0x01u << (curEp - 1u));
...
uint8 USB_ClearEndpointHalt(void) 
{
    ...
    if ((ep > USB_EP0) && (ep < USB_MAX_EP))
    {
        ...
        USB_SIE_EP_BASE.sieEp[ep].epCnt0 &= (uint32) ~(uint32)USB_EPX_CNT_DATA_TOGGLE;
