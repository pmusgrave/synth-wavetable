 ...
 * USBFS_1_GetDTERate(), USBFS_1_GetCharFormat(),
 * USBFS_1_GetParityType(), and USBFS_1_GetDataBits() APIs.
...
volatile uint8 USBFS_1_linesCoding[USBFS_1_MAX_MULTI_COM_NUM][USBFS_1_LINE_CODING_SIZE] =
{
    /*COM Port 1*/
    {
...
/**Used as a flag for the USBFS_1_IsLineChanged() API, to inform it that the
 ...
 * support. In case of 1 COM port, data is in 0 element.*/
volatile uint8  USBFS_1_linesChanged[USBFS_1_MAX_MULTI_COM_NUM];
 ...
 * USBFS_1_GetLineControl() API. It is an array of 2 elements for COM 
 * port 1 and COM port 2 for MultiCOM port support. In case of 1 COM port, data 
 * is in 0 element.*/
volatile uint16 USBFS_1_linesControlBitmap[USBFS_1_MAX_MULTI_COM_NUM];
/** Contains the 16-bit serial state value that was sent using the 
 * \ref USBFS_1_SendSerialState() API. . It is an array of 2 elements 
 * for COM port 1 and COM port 2 for MultiCOM port support. In case of 1 COM 
 * port, data is in 0 element.*/
volatile uint16 USBFS_1_serialStateBitmap[USBFS_1_MAX_MULTI_COM_NUM];
...
volatile uint8  USBFS_1_cdcDataInEp[USBFS_1_MAX_MULTI_COM_NUM];
 ...
 * port 2 for MultiCOM port support. In case of 1 COM port, data is in 0 element.*/
volatile uint8  USBFS_1_cdcDataOutEp[USBFS_1_MAX_MULTI_COM_NUM];
...
volatile uint8  USBFS_1_cdcCommInInterruptEp[USBFS_1_MAX_MULTI_COM_NUM];

/*PRIVATE*/

#define USBFS_1_CDC_IN_EP      (0u)
#define USBFS_1_CDC_OUT_EP     (1u)
#define USBFS_1_CDC_NOTE_EP    (2u)

#define USBFS_1_CDC_EP_MASK    (0x01u)

#define USBFS_1_GET_EP_COM_NUM(cdcComNums, epType) (((cdcComNums) >> (epType)) & USBFS_1_CDC_EP_MASK)
...
***************************************/
    ...
    static t_USBFS_1_cdc_notification USBFS_1_serialStateNotification =
    {

        USBFS_1_SERIAL_STATE_REQUEST_TYPE, /* bRequestType    */
        USBFS_1_SERIAL_STATE,              /* bNotification   */
        0u,                                         /* wValue          */
        0u,                                         /* wValueMSB       */
        0u,                                         /* wIndex          */
        0u,                                         /* wIndexMSB       */
        USBFS_1_SERIAL_STATE_LENGTH,       /* wLength         */
        0u,                                         /* wLengthMSB      */
        0u,                                         /* wSerialState    */
        0u,                                         /* wSerialStateMSB */
    };
    static uint8 USBFS_1_activeCom = 0u;
...
*
* \globalvars
*   USBFS_1_linesCoding: Contains the current line coding structure.
...
*   USBFS_1_linesControlBitmap: Contains the current control signal
*     bitmap. It is set by the Host using SET_CONTROL_LINE request and returned
*     to the user code by the USBFS_GetLineControl() API.
*   USBFS_1_linesChanged: This variable is used as a flag for the
...
uint8 USBFS_1_DispatchCDCClassRqst(void) 
{
    ...
    comPort = USBFS_1_GetInterfaceComPort((uint8)USBFS_1_wIndexLoReg);
    ...
    if (0u != (USBFS_1_bmRequestTypeReg & USBFS_1_RQST_DIR_D2H))
    {
        ...
        switch (USBFS_1_bRequestReg)
        {
            case USBFS_1_CDC_GET_LINE_CODING:
                ...
                USBFS_1_currentTD.pData = USBFS_1_linesCoding[comPort];
            ...
            #ifdef USBFS_1_DISPATCH_CDC_CLASS_CDC_READ_REQUESTS_CALLBACK
                requestHandled = USBFS_1_DispatchCDCClass_CDC_READ_REQUESTS_Callback();
            #endif /* (USBFS_1_DISPATCH_CDC_CLASS_CDC_READ_REQUESTS_CALLBACK) */
                break;
        }
    }
    else
    {
        ...
        switch (USBFS_1_bRequestReg)
        {
            case USBFS_1_CDC_SET_LINE_CODING:
                USBFS_1_currentTD.count  = USBFS_1_LINE_CODING_SIZE;
                USBFS_1_currentTD.pData  = USBFS_1_linesCoding[comPort];
                USBFS_1_linesChanged[comPort] |= USBFS_1_LINE_CODING_CHANGED;

                requestHandled = USBFS_1_InitControlWrite();
                break;

            case USBFS_1_CDC_SET_CONTROL_LINE_STATE:
                USBFS_1_linesControlBitmap[comPort] = (uint8) USBFS_1_wValueLoReg;
                USBFS_1_linesChanged[comPort]      |= USBFS_1_LINE_CONTROL_CHANGED;
            ...
            default:
                /* Do not handle this request unless callback is defined. */
            #ifdef USBFS_1_DISPATCH_CDC_CLASS_CDC_WRITE_REQUESTS_CALLBACK
                requestHandled = USBFS_1_DispatchCDCClass_CDC_WRITE_REQUESTS_Callback();
            #endif /* (USBFS_1_DISPATCH_CDC_CLASS_CDC_WRITE_REQUESTS_CALLBACK) */
                break;
        }
    }
...
/***************************************************************************
* Function Name: USBFS_1_GetInterfaceComPort
************************************************************************//**
*   \internal
...
uint8 USBFS_1_GetInterfaceComPort(uint8 interface) 
{
    ...
    while (comPort < USBFS_1_MAX_MULTI_COM_NUM)
    {
        notEp = USBFS_1_cdcCommInInterruptEp[comPort];

        if (USBFS_1_EP[notEp].interface == interface)
        {
            res = comPort;
            comPort = USBFS_1_MAX_MULTI_COM_NUM;
        }
...
*   USBFS_1_SUCCESS   CDC interface was initialized correctly
*   USBFS_1_FAILURE   CDC interface was not initialized
*
* \globalvars
*   USBFS_1_linesChanged: Initialized to zero.
*   USBFS_1_cdcDataOutEp: Used as an OUT endpoint number.
*
* \reentrant
*  No.
*
...
uint8 USBFS_1_CDC_Init(void) 
{
    uint8 comPort;
    uint8 outEp;
    uint8 ret = USBFS_1_SUCCESS;

    USBFS_1_activeCom = 0u;
    USBFS_1_linesChanged[USBFS_1_COM_PORT1] = 0u;
    USBFS_1_linesChanged[USBFS_1_COM_PORT2] = 0u;

    for(comPort = 0u; comPort<USBFS_1_MAX_MULTI_COM_NUM; comPort++)
    {
        outEp = USBFS_1_cdcDataOutEp[comPort];
        if((0u != outEp) && (USBFS_1_MAX_EP > outEp))
        {
            USBFS_1_EnableOutEP(outEp);
        }

    }

    /* COM Port 1 should be correct to proceed. */
    if ((0u == USBFS_1_cdcDataInEp[USBFS_1_COM_PORT1]) \
            || (0u == USBFS_1_cdcDataOutEp[USBFS_1_COM_PORT1]) \
            || (0u ==  USBFS_1_cdcCommInInterruptEp[USBFS_1_COM_PORT1])
            || (USBFS_1_cdcDataInEp[USBFS_1_COM_PORT1] >= USBFS_1_MAX_EP)
            || (USBFS_1_cdcDataOutEp[USBFS_1_COM_PORT1] >= USBFS_1_MAX_EP)
            || (USBFS_1_cdcCommInInterruptEp[USBFS_1_COM_PORT1] >= USBFS_1_MAX_EP))
    {
        ret = USBFS_1_FAILURE;
    }
...
* \globalvars
*
*   USBFS_1_cdcDataInEp: CDC IN endpoint number used for sending
*     data.
*
* \reentrant
*  No.
...
{
    uint8 epNumber = USBFS_1_cdcDataInEp[USBFS_1_activeCom];

    /* Limit length to maximum packet size for endpoint. */
    if (length > USBFS_1_EP[epNumber].bufferSize)
    {
        /* Caution: Data will be lost if length is greater than Max Packet Size. */
        length = USBFS_1_EP[epNumber].bufferSize;

        /* Halt CPU in debug mode */
        CYASSERT(0u != 0u);
    }

    USBFS_1_LoadInEP(epNumber, pData, length);
}
...
*  The USBFS_1_CDCIsReady() function should be called before
*  sending data with a new call to USBFS_1_PutString(), to be sure
...
void USBFS_1_PutString(const char8 string[]) 
{
    ...
    do
    {
        ...
        sendLength = (strLength > USBFS_1_EP[epNumber].bufferSize) ?
                                  USBFS_1_EP[epNumber].bufferSize : strLength;

        /* Load IN endpoint and expose it to host. */
        USBFS_1_LoadInEP(epNumber, (const uint8 *)&string[bufIndex], sendLength);
        strLength -= sendLength;

        /* If more data are present to send or full packet was sent */
        if ((strLength > 0u) || (sendLength == USBFS_1_EP[epNumber].bufferSize))
        {
            bufIndex += sendLength;

            /* Wait until host read data from IN endpoint buffer. */
            while (USBFS_1_IN_BUFFER_FULL == USBFS_1_EP[epNumber].apiEpState)
            {
            }
            ...
            */
            if (0u == strLength)
            {
                USBFS_1_LoadInEP(epNumber, NULL, 0u);
            }
        }
    }
    while (strLength > 0u);
...
void USBFS_1_PutChar(char8 txDataByte) 
{
    ...
    USBFS_1_LoadInEP(USBFS_1_cdcDataInEp[USBFS_1_activeCom], &dataByte, 1u);
}
...
void USBFS_1_PutCRLF(void) 
{
    ...
    USBFS_1_LoadInEP(USBFS_1_cdcDataInEp[USBFS_1_activeCom], (const uint8 *)txData, 2u);
}
...
*
* \globalvars
*   USBFS_1_cdcDataOutEp: CDC OUT endpoint number used.
*
*******************************************************************************/
uint16 USBFS_1_GetCount(void) 
{
    ...
    uint8  epNumber = USBFS_1_cdcDataOutEp[USBFS_1_activeCom];

    if (USBFS_1_OUT_BUFFER_FULL == USBFS_1_EP[epNumber].apiEpState)
    {
        bytesCount = USBFS_1_GetEPCount(epNumber);
    }
    else
    {
        bytesCount = 0u;
...
*  received zero-length packet. The USBFS_1_GetAll() or
...
{
    return (USBFS_1_GetEPState(USBFS_1_cdcDataOutEp[USBFS_1_activeCom]));
}
...
*
* \globalvars
*   USBFS_1_cdcDataInEp: CDC IN endpoint number used.
*
*******************************************************************************/
uint8 USBFS_1_CDCIsReady(void) 
{
    return (USBFS_1_GetEPState(USBFS_1_cdcDataInEp[USBFS_1_activeCom]));
}
...
uint16 USBFS_1_GetData(uint8* pData, uint16 length) 
{
    ...
    length = USBFS_1_ReadOutEP(epNumber, pData, length);

#if (USBFS_1_EP_MANAGEMENT_DMA_MANUAL)
    /* Wait until DMA complete transferring data from OUT endpoint buffer. */
    while (USBFS_1_OUT_BUFFER_FULL == USBFS_1_GetEPState(epNumber))
    {
    }
...
*  USBFS_1_DataIsReady() API should be called first, to be sure
...
*
* \globalvars
*   - \ref USBFS_1_cdcDataOutEp: CDC OUT endpoint number used.
*   - \ref USBFS_1_EP[].bufferSize: EP max packet size is used as a
...
uint16 USBFS_1_GetAll(uint8* pData) 
{
    ...
    /* Read data from OUT endpoint buffer. */
    dataLength = USBFS_1_ReadOutEP(epNumber, pData, USBFS_1_EP[epNumber].bufferSize);
...
uint8 USBFS_1_GetChar(void) 
{
    ...
    (void) USBFS_1_ReadOutEP(epNumber, &rxData, 1u);
...
*
* \globalvars
*  - \ref USBFS_1_transferState: it is checked to be sure then OUT
*    data phase has been complete, and data written to the lineCoding or
*    Control Bitmap buffer.
*  - \ref USBFS_1_linesChanged: used as a flag to be aware that
...
uint8 USBFS_1_IsLineChanged(void) 
{
    ...
    {
        if (USBFS_1_linesChanged[USBFS_1_activeCom] != 0u)
        {
            state = USBFS_1_linesChanged[USBFS_1_activeCom];
            USBFS_1_linesChanged[USBFS_1_activeCom] = 0u;
        }
    }
...
*
* \globalvars
*  USBFS_1_linesCoding: First four bytes converted to uint32
...
uint32 USBFS_1_GetDTERate(void) 
{
    ...
    rate = USBFS_1_linesCoding[USBFS_1_activeCom][USBFS_1_LINE_CODING_RATE + 3u];
    rate = (rate << 8u) | USBFS_1_linesCoding[USBFS_1_activeCom][USBFS_1_LINE_CODING_RATE + 2u];
    rate = (rate << 8u) | USBFS_1_linesCoding[USBFS_1_activeCom][USBFS_1_LINE_CODING_RATE + 1u];
    rate = (rate << 8u) | USBFS_1_linesCoding[USBFS_1_activeCom][USBFS_1_LINE_CODING_RATE];
...
*
* \globalvars
*  USBFS_1_linesCoding: used to get a parameter.
*
*******************************************************************************/
uint8 USBFS_1_GetCharFormat(void) 
{
    return (USBFS_1_linesCoding[USBFS_1_activeCom][USBFS_1_LINE_CODING_STOP_BITS]);
}
...
{
    return (USBFS_1_linesCoding[USBFS_1_activeCom][USBFS_1_LINE_CODING_PARITY]);
}
...
{
    return (USBFS_1_linesCoding[USBFS_1_activeCom][USBFS_1_LINE_CODING_DATA_BITS]);
}
...
*
* \globalvars
*  USBFS_1_linesControlBitmap: used to get a parameter.
*
*******************************************************************************/
uint16 USBFS_1_GetLineControl(void) 
{
    return (USBFS_1_linesControlBitmap[USBFS_1_activeCom]);
}


/*******************************************************************************
* Function Name: USBFS_1_SendSerialState
****************************************************************************//**
*
...
*  USBFS_1_NotificationIsReady() API must be called to check if the
...
{
    uint8 epNumber = USBFS_1_cdcCommInInterruptEp[USBFS_1_activeCom];

    if(USBFS_1_SERIAL_STATE_SIZE <= USBFS_1_EP[epNumber].bufferSize)
    {
        /* Save current SERIAL_STATE bitmap. */
        USBFS_1_serialStateBitmap[USBFS_1_activeCom] = serialState;

        /* Add interface number */
        USBFS_1_serialStateNotification.wIndex = USBFS_1_EP[epNumber].interface;

        /*Form SERIAL_STATE data*/
        USBFS_1_serialStateNotification.wSerialState =    LO8(USBFS_1_serialStateBitmap[USBFS_1_activeCom]);
        USBFS_1_serialStateNotification.wSerialStateMSB = HI8(USBFS_1_serialStateBitmap[USBFS_1_activeCom]);

        USBFS_1_LoadInEP(epNumber, (uint8 *) &USBFS_1_serialStateNotification, sizeof(USBFS_1_serialStateNotification));
    }
}


/*******************************************************************************
* Function Name: USBFS_1_GetSerialState
****************************************************************************//**
*
...
{
    return USBFS_1_serialStateBitmap[USBFS_1_activeCom];
}


/*******************************************************************************
* Function Name: USBFS_1_NotificationIsReady
****************************************************************************//**
*
...
*  USBFS_1_SendSerialState() to ensure that any previous
...
{
    return (USBFS_1_EP[USBFS_1_cdcCommInInterruptEp[USBFS_1_activeCom]].apiEpState);
}


/*******************************************************************************
* Function Name: USBFS_1_SetComPort
****************************************************************************//**
*
...
{
    if ((USBFS_1_activeCom != comNumber) && \
            (comNumber < USBFS_1_MAX_MULTI_COM_NUM ))
    {
        USBFS_1_activeCom = comNumber;
    }
}


/*******************************************************************************
* Function Name: USBFS_1_GetComPort
****************************************************************************//**
*
...
/***************************************************************************
* Function Name: USBFS_1_Cdc_EpInit
************************************************************************//**
*
*  \internal
...
*   USBFS_1_cdcDataInEp[], USBFS_1_cdcCommInInterruptEp[],
*   USBFS_1_cdcDataOutEp[]
*
...
uint8 USBFS_1_Cdc_EpInit(const T_USBFS_1_EP_SETTINGS_BLOCK CYCODE *pEP, uint8 epNum, uint8 cdcComNums) 
{
    ...
    {
        if (epType != USBFS_1_EP_TYPE_INT)
        {
            USBFS_1_cdcDataInEp[USBFS_1_GET_EP_COM_NUM(cdcComNums, USBFS_1_CDC_IN_EP)] = epNum;
            cdcComNums |= (uint8)(USBFS_1_COM_PORT2 << USBFS_1_CDC_IN_EP);
        }
        else
        {

            USBFS_1_cdcCommInInterruptEp[USBFS_1_GET_EP_COM_NUM(cdcComNums, USBFS_1_CDC_NOTE_EP)] = epNum;
            cdcComNums |= (uint8)(USBFS_1_COM_PORT2 << USBFS_1_CDC_NOTE_EP);
        }
    }
    else
    {
        if (epType != USBFS_1_EP_TYPE_INT)
        {
            USBFS_1_cdcDataOutEp[USBFS_1_GET_EP_COM_NUM(cdcComNums, USBFS_1_CDC_OUT_EP)] = epNum;
            cdcComNums |= (uint8)(USBFS_1_COM_PORT2 << USBFS_1_CDC_OUT_EP);
        }
    }
    return (cdcComNums);
}
