...
uint8 USBMIDI_1_HandleStandardRqst(void) 
{
    ...
    uint32 bmRequestType = USBMIDI_1_bmRequestTypeReg;
...
void USBMIDI_1_ConfigReg(void) 
{
    ...
    USBMIDI_1_ARB_CFG_REG |= (uint32)  USBMIDI_1_ARB_CFG_CFG_CMP;
    ...
    USBMIDI_1_SIE_EP_INT_EN_REG = (uint32) USBMIDI_1_DEFAULT_SIE_EP_INT_EN;
}
    ...
    if (USBMIDI_1_configuration > 0u)
    {
        ...
        */
        ...
        for (i = 0u; i < ep; i++)
        {
            ...
            {
                curEp  = ((uint32) pEP->addr & USBMIDI_1_DIR_UNUSED);
                epType = ((uint32) pEP->attributes & USBMIDI_1_EP_TYPE_MASK);
            ...
            #if (USBMIDI_1_EP_MANAGEMENT_DMA_AUTO)
                /* Clear IN data ready. */
                USBMIDI_1_ARB_EP_BASE.arbEp[curEp].epCfg &= (uint32) ~USBMIDI_1_ARB_EPX_CFG_IN_DATA_RDY;

                /* Select endpoint number of reconfiguration */
                USBMIDI_1_DYN_RECONFIG_REG = (uint32) ((curEp - 1u) << USBMIDI_1_DYN_RECONFIG_EP_SHIFT);
                ...
                if (0u != (pEP->addr & USBMIDI_1_DIR_IN))
                {
                    ...
                    USBMIDI_1_EP_TYPE_REG &= (uint32) ~(uint32)((uint32) 0x01u << (curEp - 1u));
                ...
                {
                    /* Set endpoint type: 0 - IN and 1- OUT. */
                    USBMIDI_1_EP_TYPE_REG |= (uint32) ((uint32) 0x01u << (curEp - 1u));
                ...
                */
                USBMIDI_1_DYN_RECONFIG_REG &= (uint32) ~USBMIDI_1_DYN_RECONFIG_ENABLE;
...
uint8 USBMIDI_1_ClearEndpointHalt(void) 
{
    ...
    if ((ep > USBMIDI_1_EP0) && (ep < USBMIDI_1_MAX_EP))
    {
        ...
        USBMIDI_1_SIE_EP_BASE.sieEp[ep].epCnt0 = (uint32) ~(uint32)USBMIDI_1_EPX_CNT_DATA_TOGGLE;
