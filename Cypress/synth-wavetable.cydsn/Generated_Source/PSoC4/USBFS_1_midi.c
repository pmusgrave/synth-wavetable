...
***************************************/
    ...
    volatile uint8 USBFS_1_midi_in_ep;
    ...
    volatile uint8 USBFS_1_MIDI1_InqFlags;                                 /* Device inquiry flag */
        ...
        /** See description of \ref USBFS_1_MIDI1_InqFlags*/
...
* \globalvars
*
*  \ref USBFS_1_midiInBuffer: This buffer is used for saving and combining
...
*
*  \ref USBFS_1_midiOutBuffer: This buffer is used by the
*    USBFS_1_MIDI_OUT_Service() function for saving the received
*    from the PC data, then the data are parsed and transferred to UART(s)
*    buffer and to the internal processing by the
*
*  \ref USBFS_1_callbackLocalMidiEvent function.
*
*  \ref USBFS_1_midi_out_ep: Used as an OUT endpoint number.
*
*  \ref USBFS_1_midi_in_ep: Used as an IN endpoint number.
*
*   \ref USBFS_1_midiInPointer: Initialized to zero.
*
* \sideeffect
...
void USBFS_1_MIDI_Init(void) 
{
...
#if (USBFS_1_MIDI_OUT_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: USBFS_1_MIDI_OUT_Service
    ****************************************************************************//**
    *
    ...
    * \globalvars
    *
    *  \ref USBFS_1_midiOutBuffer: Used as temporary buffer between USB
    ...
    *
    *******************************************************************************/
    void USBFS_1_MIDI_OUT_Service(void) 
    {
        ...
        uint8 outLength;
        uint8 outPointer;
    #endif /* (USBFS_1_MIDI_OUT_BUFF_SIZE >= 256) */

        /* Service the USB MIDI output endpoint. */
        if (USBFS_1_OUT_BUFFER_FULL == USBFS_1_GetEPState(USBFS_1_midi_out_ep))
        {
        #if (USBFS_1_MIDI_OUT_BUFF_SIZE >= 256)
            ...
            outLength = (uint8)USBFS_1_GetEPCount(USBFS_1_midi_out_ep);
                ...
                while (USBFS_1_OUT_BUFFER_FULL == USBFS_1_GetEPState(USBFS_1_midi_out_ep))
                {
                }
            ...
            if (outLength >= USBFS_1_EVENT_LENGTH)
            {
                ...
                /* Route USB MIDI to the External connection */
                    ...
                    else
                    {
                        ...
                        #ifdef USBFS_1_MIDI_OUT_EP_SERVICE_CALLBACK
                            USBFS_1_MIDI_OUT_EP_Service_Callback();
                        #endif /* USBFS_1_MIDI_OUT_EP_SERVICE_CALLBACK */
                    }
                #endif /* (USBFS_1_MIDI_EXT_MODE >= USBFS_1_ONE_EXT_INTRF) */

                    /* Process any local MIDI output functions */
                    USBFS_1_callbackLocalMidiEvent(USBFS_1_midiOutBuffer[outPointer] & USBFS_1_CABLE_MASK,
    ...
    *  The USBFS_1_MIDI_IN_Service() function should also be called to
    ...
    *   ----------------------|-----------------------------------------
    *  USBFS_1_TRUE  | Host is not ready to receive this message
    *  USBFS_1_FALSE | Success transfer
    *
    * \globalvars
    *
    *  \ref USBFS_1_midi_in_ep: MIDI IN endpoint number used for
    *        sending data.
    *
    *  \ref USBFS_1_midiInPointer: Checked this variable to see if
                    ...
                    (USBFS_1_EP[USBFS_1_midi_in_ep].bufferSize - USBFS_1_EVENT_LENGTH))
        {
            ...
            else /* Only SysEx message is greater than 4 bytes */
            {
                ...
                if (retError == USBFS_1_FALSE)
                {
                    ...
                    USBFS_1_PrepareInBuffer(USBFS_1_MIDI_EOSEX, &midiMsg[idx], ic, cable);
                }
            }
        }
        else
    ...
    *******************************************************************************/
                                                                 ...
                                                                 
    {
        ...
        else if (ic == USBFS_1_MIDI_EOSEX)
        {
            ...
            {
                case 0u:
                    USBFS_1_midiInBuffer[USBFS_1_midiInPointer] = USBFS_1_SYSEX_ENDS_WITH1 | cable;
                    USBFS_1_midiInPointer++;
                    ...
                    break;
                case 1u:
                    USBFS_1_midiInBuffer[USBFS_1_midiInPointer] = USBFS_1_SYSEX_ENDS_WITH2 | cable;
                    USBFS_1_midiInPointer++;
                    USBFS_1_midiInBuffer[USBFS_1_midiInPointer] = srcBuffZero;
                    USBFS_1_midiInPointer++;
                    USBFS_1_midiInBuffer[USBFS_1_midiInPointer] = USBFS_1_MIDI_EOSEX;
                    ...
                    break;
                case 2u:
                    USBFS_1_midiInBuffer[USBFS_1_midiInPointer] = USBFS_1_SYSEX_ENDS_WITH3 | cable;
                    USBFS_1_midiInPointer++;
                    USBFS_1_midiInBuffer[USBFS_1_midiInPointer] = srcBuffZero;
                    USBFS_1_midiInPointer++;
                    USBFS_1_midiInBuffer[USBFS_1_midiInPointer] = srcBuffOne;
...
*/
#if (USBFS_1_MIDI_EXT_MODE >= USBFS_1_ONE_EXT_INTRF)
    /*******************************************************************************
    * Function Name: USBFS_1_MIDI_InitInterface
    ****************************************************************************//**
    *
    ...
    *
    *******************************************************************************/
    void USBFS_1_MIDI_InitInterface(void) 
    {
    ...
    #ifdef USBFS_1_MIDI_INIT_CALLBACK
        USBFS_1_MIDI_Init_Callback();
    #endif /* (USBFS_1_MIDI_INIT_CALLBACK) */
    }
    ...
    *******************************************************************************/
    ...
    #ifdef USBFS_1_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK
        USBFS_1_MIDI1_ProcessUsbOut_EntryCallback();
    #endif /* (USBFS_1_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK) */
    ...
    #ifdef USBFS_1_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK
        USBFS_1_MIDI1_ProcessUsbOut_ExitCallback();
    #endif /* (USBFS_1_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }
    ...
    *******************************************************************************/
    ...
    #ifdef USBFS_1_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK
        USBFS_1_MIDI2_ProcessUsbOut_EntryCallback();
    #endif /* (USBFS_1_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK) */
    ...
    #ifdef USBFS_1_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK
        USBFS_1_MIDI2_ProcessUsbOut_ExitCallback();
    #endif /* (USBFS_1_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }
#endif /* (USBFS_1_MIDI_EXT_MODE >= USBFS_1_TWO_EXT_INTRF) */
#endif /* (USBFS_1_MIDI_EXT_MODE >= USBFS_1_ONE_EXT_INTRF) */

#endif  /*  (USBFS_1_ENABLE_MIDI_API != 0u) */
...
#endif  /* defined(USBFS_1_ENABLE_MIDI_STREAMING) */
