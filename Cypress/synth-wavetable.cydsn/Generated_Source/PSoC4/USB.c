...
void USB_Stop(void) 
{
    ...
    USB_CR0_REG &= (uint32) ~USB_CR0_ENABLE;

    /* Disable D+ pull-up. */
    USB_USBIO_CR1_REG &= (uint32) ~ USB_USBIO_CR1_USBPUEN;
...
{
    uint32 cr1Reg = USB_CR1_REG;

    /* Clear bus activity. */
    USB_CR1_REG = (cr1Reg & (uint32) ~USB_CR1_BUS_ACTIVITY);
