...
{
    CyRamVectors[CYINT_IRQ_BASE + isr_TxDMADone__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + isr_TxDMADone__INTC_NUMBER];
}
...
void isr_TxDMADone_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((isr_TxDMADone__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *isr_TxDMADone_INTC_PRIOR = (*isr_TxDMADone_INTC_PRIOR & (uint32)(~isr_TxDMADone__INTC_PRIOR_MASK)) |
...
uint8 isr_TxDMADone_GetPriority(void)
{
    ...
    priority = (*isr_TxDMADone_INTC_PRIOR & isr_TxDMADone__INTC_PRIOR_MASK) >> priorityOffset;
