...
/*******************************************************************************
* Function Name: CodecI2CM_I2C_ISR
****************************************************************************//**
*
*  Handles the Interrupt Service Routine for the SCB I2C mode.
*
*******************************************************************************/
CY_ISR(CodecI2CM_I2C_ISR)
{
    uint32 diffCount;
    uint32 endTransfer;

#ifdef CodecI2CM_I2C_ISR_ENTRY_CALLBACK
    CodecI2CM_I2C_ISR_EntryCallback();
#endif /* CodecI2CM_I2C_ISR_ENTRY_CALLBACK */

#if (CodecI2CM_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = CodecI2CM_I2C_ACK_ADDR;
    ...
    if(NULL != CodecI2CM_customIntrHandler)
    {
        CodecI2CM_customIntrHandler();
    }

    if(CodecI2CM_CHECK_INTR_I2C_EC_MASKED(CodecI2CM_INTR_I2C_EC_WAKE_UP))
    {
    ...
    */
    if(CodecI2CM_CHECK_I2C_FSM_MASTER)
    {
        #if(CodecI2CM_I2C_MASTER)
        {
            ...
            */
            if(CodecI2CM_CHECK_INTR_MASTER_MASKED(CodecI2CM_INTR_MASTER_I2C_BUS_ERROR))
            {
                CodecI2CM_mstrStatus |= (uint16) (CodecI2CM_I2C_MSTAT_ERR_XFER |
                                                         CodecI2CM_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = CodecI2CM_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(CodecI2CM_CHECK_INTR_MASTER_MASKED(CodecI2CM_INTR_MASTER_I2C_ARB_LOST))
            {
                CodecI2CM_mstrStatus |= (uint16) (CodecI2CM_I2C_MSTAT_ERR_XFER |
                                                         CodecI2CM_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = CodecI2CM_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(CodecI2CM_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(CodecI2CM_CHECK_I2C_MASTER_CMD(CodecI2CM_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    CodecI2CM_mstrStatus |= (uint16) (CodecI2CM_I2C_MSTAT_ERR_XFER |
                                                             CodecI2CM_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                CodecI2CM_mstrStatus |= (uint16) CodecI2CM_GET_I2C_MSTAT_CMPLT;
                ...
                #if(CodecI2CM_I2C_MULTI_MASTER_SLAVE)
                {
                    if(CodecI2CM_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(CodecI2CM_CHECK_I2C_MASTER_CMD(CodecI2CM_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            CodecI2CM_I2C_MASTER_CLEAR_START;

                            endTransfer = CodecI2CM_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!CodecI2CM_CHECK_INTR_SLAVE_MASKED(CodecI2CM_INTR_SLAVE_I2C_BUS_ERROR))
                               && CodecI2CM_CHECK_INTR_MASTER_MASKED(CodecI2CM_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = CodecI2CM_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            CodecI2CM_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            CodecI2CM_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */
                        ...
                        else
                        {
                            ...
                            CodecI2CM_state = CodecI2CM_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (CodecI2CM_CHECK_I2C_FSM_SLAVE) */
    {
        #if(CodecI2CM_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or CodecI2CM_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(CodecI2CM_CHECK_INTR_SLAVE_MASKED(CodecI2CM_INTR_SLAVE_I2C_BUS_ERROR |
                                                        CodecI2CM_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(CodecI2CM_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    CodecI2CM_slStatus &= (uint8) ~CodecI2CM_I2C_SSTAT_RD_BUSY;
                    CodecI2CM_slStatus |= (uint8) (CodecI2CM_I2C_SSTAT_RD_ERR |
                                                          CodecI2CM_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    CodecI2CM_slStatus &= (uint8) ~CodecI2CM_I2C_SSTAT_WR_BUSY;
                    CodecI2CM_slStatus |= (uint8) (CodecI2CM_I2C_SSTAT_WR_ERR |
                                                          CodecI2CM_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(CodecI2CM_CHECK_I2C_FSM_MASTER)
    {
        #if(CodecI2CM_I2C_MASTER)
        {
            ...
            */
            if(CodecI2CM_CHECK_INTR_MASTER_MASKED(CodecI2CM_INTR_MASTER_I2C_STOP))
            {
            ...
            {
                if(CodecI2CM_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(CodecI2CM_CHECK_INTR_MASTER_MASKED(CodecI2CM_INTR_MASTER_I2C_NACK))
                    {
                        CodecI2CM_ClearMasterInterruptSource(CodecI2CM_INTR_MASTER_I2C_NACK);

                        CodecI2CM_mstrStatus |= (uint16) (CodecI2CM_I2C_MSTAT_ERR_XFER |
                                                                 CodecI2CM_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(CodecI2CM_CHECK_I2C_FSM_RD) /* Reading */
                        {
                        ...
                        else /* Writing */
                        {
                            ...
                            if(0u != CodecI2CM_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                CodecI2CM_SetTxInterruptMode(CodecI2CM_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(CodecI2CM_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(CodecI2CM_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(CodecI2CM_CHECK_INTR_RX_MASKED(CodecI2CM_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  CodecI2CM_mstrRdBufSize -
                                        (CodecI2CM_mstrRdBufIndex + CodecI2CM_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > CodecI2CM_I2C_FIFO_SIZE)
                            {
                                diffCount = CodecI2CM_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    CodecI2CM_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                CodecI2CM_mstrRdBufPtr[CodecI2CM_mstrRdBufIndex] = (uint8)
                                                                                        CodecI2CM_RX_FIFO_RD_REG;
                                CodecI2CM_mstrRdBufIndex++;
                            }
                        }
                        /* INTR_RX_NOT_EMPTY:
                        ...
                        */
                        else if(CodecI2CM_CHECK_INTR_RX_MASKED(CodecI2CM_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            CodecI2CM_mstrRdBufPtr[CodecI2CM_mstrRdBufIndex] = (uint8) CodecI2CM_RX_FIFO_RD_REG;
                            CodecI2CM_mstrRdBufIndex++;

                            if(CodecI2CM_mstrRdBufIndex < CodecI2CM_mstrRdBufSize)
                            {
                    ...
                    else /* Writing */
                    {
                        ...
                        */
                        if(CodecI2CM_CHECK_INTR_MASTER_MASKED(CodecI2CM_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            CodecI2CM_mstrWrBufIndexTmp -= (CodecI2CM_GET_TX_FIFO_ENTRIES +
                                                                   CodecI2CM_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            CodecI2CM_mstrWrBufIndex = CodecI2CM_mstrWrBufIndexTmp;

                            CodecI2CM_mstrStatus |= (uint16) (CodecI2CM_I2C_MSTAT_ERR_XFER |
                                                                     CodecI2CM_I2C_MSTAT_ERR_SHORT_XFER);

                            CodecI2CM_CLEAR_TX_FIFO;
                        ...
                        */
                        else if(CodecI2CM_CHECK_INTR_TX_MASKED(CodecI2CM_INTR_TX_EMPTY))
                        {
                            while(CodecI2CM_I2C_FIFO_SIZE != CodecI2CM_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(CodecI2CM_mstrWrBufIndexTmp < CodecI2CM_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(CodecI2CM_mstrWrBufIndexTmp == (CodecI2CM_mstrWrBufSize - 1u))
                                    {
                                    ...
                                    CodecI2CM_TX_FIFO_WR_REG = (uint32) CodecI2CM_mstrWrBufPtr[CodecI2CM_mstrWrBufIndexTmp];
                                    CodecI2CM_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                        ...
                        #else
                            CodecI2CM_ClearTxInterruptSource(CodecI2CM_INTR_TX_EMPTY);
                        #endif /* (CodecI2CM_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(CodecI2CM_CHECK_INTR_TX_MASKED(CodecI2CM_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            CodecI2CM_mstrWrBufIndex = CodecI2CM_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(CodecI2CM_CHECK_I2C_MODE_NO_STOP(CodecI2CM_mstrControl))
                    {
        ...
        } /* (CodecI2CM_I2C_MASTER) */
        #endif

    } /* (CodecI2CM_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(CodecI2CM_CHECK_I2C_FSM_SLAVE)
    {
        #if(CodecI2CM_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(CodecI2CM_CHECK_INTR_SLAVE_MASKED(CodecI2CM_INTR_SLAVE_I2C_NACK))
            {
                CodecI2CM_ClearSlaveInterruptSource(CodecI2CM_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (CodecI2CM_GET_TX_FIFO_ENTRIES + CodecI2CM_GET_TX_FIFO_SR_VALID);

                if(CodecI2CM_slOverFlowCount > diffCount) /* Overflow */
                {
                    CodecI2CM_slStatus |= (uint8) CodecI2CM_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    CodecI2CM_slRdBufIndexTmp -= (diffCount - CodecI2CM_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                CodecI2CM_slRdBufIndex = CodecI2CM_slRdBufIndexTmp;
                ...
                CodecI2CM_slStatus &= (uint8) ~CodecI2CM_I2C_SSTAT_RD_BUSY;
                CodecI2CM_slStatus |= (uint8)  CodecI2CM_I2C_SSTAT_RD_CMPLT;
                CodecI2CM_state     =  CodecI2CM_I2C_FSM_IDLE;
                
            #ifdef CodecI2CM_I2C_SLAVE_CMPLT_CALLBACK
                /* Read complete */
                CodecI2CM_I2C_SlaveCompleteCallback();
            #endif /* CodecI2CM_I2C_SLAVE_CMPLT_CALLBACK */
            }
            ...
            */
            if(CodecI2CM_CHECK_INTR_SLAVE_MASKED(CodecI2CM_INTR_SLAVE_I2C_WRITE_STOP))
            {
                CodecI2CM_ClearSlaveInterruptSource(CodecI2CM_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (CodecI2CM_I2C_CTRL_REG & CodecI2CM_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != CodecI2CM_GET_RX_FIFO_ENTRIES)
                    {
                        #if(CodecI2CM_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == CodecI2CM_GET_RX_FIFO_ENTRIES) &&
                               (CodecI2CM_CHECK_INTR_SLAVE_MASKED(CodecI2CM_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        CodecI2CM_slWrBufPtr[CodecI2CM_slWrBufIndex] = (uint8) CodecI2CM_RX_FIFO_RD_REG;
                        CodecI2CM_slWrBufIndex++;
                    }

                    CodecI2CM_DISABLE_SLAVE_AUTO_DATA;
                }

                if(CodecI2CM_CHECK_INTR_RX(CodecI2CM_INTR_RX_OVERFLOW))
                {
                    CodecI2CM_slStatus |= (uint8) CodecI2CM_I2C_SSTAT_WR_OVFL;
                }
                ...
                CodecI2CM_slStatus &= (uint8) ~CodecI2CM_I2C_SSTAT_WR_BUSY;
                CodecI2CM_slStatus |= (uint8)  CodecI2CM_I2C_SSTAT_WR_CMPLT;
                CodecI2CM_state     =  CodecI2CM_I2C_FSM_IDLE;
            ...
            */
        ...
        #endif /* (CodecI2CM_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                CodecI2CM_ClearI2CExtClkInterruptSource(CodecI2CM_INTR_I2C_EC_WAKE_UP);

                #if (CodecI2CM_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != CodecI2CM_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = CodecI2CM_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) CodecI2CM_RX_FIFO_RD_REG;
                        response = CodecI2CM_I2C_ACK_ADDR;
                    }
            ...
            #if (CodecI2CM_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == CodecI2CM_I2C_NAK_ADDR)
                {
                #if (!CodecI2CM_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    CodecI2CM_DISABLE_INTR_SLAVE(CodecI2CM_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!CodecI2CM_CY_SCBIP_V0) */
                    ...
                    CodecI2CM_I2C_SLAVE_GENERATE_NACK;
                }
                else
            #endif /* (CodecI2CM_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    ...
                    /* TX direction: master reads from slave */
                    {
                        ...
                        CodecI2CM_slRdBufIndexTmp = CodecI2CM_slRdBufIndex;

                        /* Start master reading */
                        CodecI2CM_slStatus |= (uint8) CodecI2CM_I2C_SSTAT_RD_BUSY;
                        CodecI2CM_state     = CodecI2CM_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (CodecI2CM_slWrBufSize - CodecI2CM_slWrBufIndex);
                        ...
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == CodecI2CM_I2C_FIFO_SIZE)
                            {
                                ...
                                CodecI2CM_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                CodecI2CM_ENABLE_SLAVE_AUTO_DATA_ACK;
                        ...
                        /* Start master reading */
                        CodecI2CM_slStatus |= (uint8) CodecI2CM_I2C_SSTAT_WR_BUSY;
                        CodecI2CM_state     = CodecI2CM_I2C_FSM_SL_WR;
                    }
                    ...
                    CodecI2CM_ENABLE_INTR_SLAVE(CodecI2CM_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!CodecI2CM_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    CodecI2CM_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(CodecI2CM_CHECK_INTR_RX_MASKED(CodecI2CM_INTR_RX_FULL))
            {
                ...
                diffCount =  CodecI2CM_slWrBufSize -
                            (CodecI2CM_slWrBufIndex + CodecI2CM_I2C_FIFO_SIZE);

                if(diffCount > CodecI2CM_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = CodecI2CM_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    CodecI2CM_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                CodecI2CM_ClearRxInterruptSource(CodecI2CM_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = CodecI2CM_RX_FIFO_RD_REG;

                if(CodecI2CM_slWrBufIndex < CodecI2CM_slWrBufSize)
                {
                    CodecI2CM_I2C_SLAVE_GENERATE_ACK;

                    /* Put data into component buffer */
                    CodecI2CM_slWrBufPtr[CodecI2CM_slWrBufIndex] = (uint8) diffCount;
                    CodecI2CM_slWrBufIndex++;
                }
            ...
            */
            if(CodecI2CM_CHECK_INTR_TX_MASKED(CodecI2CM_INTR_TX_EMPTY))
            {
                while(CodecI2CM_I2C_FIFO_SIZE != CodecI2CM_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(CodecI2CM_slRdBufIndexTmp < CodecI2CM_slRdBufSize)
                    /* Data from buffer */
                    {
                        CodecI2CM_TX_FIFO_WR_REG = (uint32) CodecI2CM_slRdBufPtr[CodecI2CM_slRdBufIndexTmp];
                        CodecI2CM_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        CodecI2CM_TX_FIFO_WR_REG = CodecI2CM_I2C_SLAVE_OVFL_RETURN;

                        if(CodecI2CM_slOverFlowCount <= CodecI2CM_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            CodecI2CM_slOverFlowCount++;
                        }
                    }
                }
...
#ifdef CodecI2CM_I2C_ISR_EXIT_CALLBACK
    CodecI2CM_I2C_ISR_ExitCallback();
#endif /* CodecI2CM_I2C_ISR_EXIT_CALLBACK */
