...
#include "spi_ss.h"
    ...
    #define spi_ss_SetP4PinDriveMode(shift, mode)  \
    do { \
        spi_ss_PC =   (spi_ss_PC & \
                                (uint32)(~(uint32)(spi_ss_DRIVE_MODE_IND_MASK << \
                                (spi_ss_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (spi_ss_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define spi_ss_SetP4PinDriveMode(shift, mode)  \
        do { \
            spi_ss_USBIO_CTRL_REG = (spi_ss_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(spi_ss_DRIVE_MODE_IND_MASK << \
                                    (spi_ss_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (spi_ss_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		spi_ss_SetP4PinDriveMode(spi_ss__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(spi_ss_DR & (uint8)(~spi_ss_MASK));
    drVal = (drVal | ((uint8)(value << spi_ss_SHIFT) & spi_ss_MASK));
    spi_ss_DR = (uint32)drVal;
}
...
{
    return (uint8)((spi_ss_PS & spi_ss_MASK) >> spi_ss_SHIFT);
}
...
{
    return (uint8)((spi_ss_DR & spi_ss_MASK) >> spi_ss_SHIFT);
}
...
void spi_ss_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  spi_ss_INTCFG & (uint32)(~(uint32)position);
    spi_ss_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(spi_ss_INTSTAT & spi_ss_MASK);
	spi_ss_INTSTAT = maskedStatus;
    return maskedStatus >> spi_ss_SHIFT;
}
