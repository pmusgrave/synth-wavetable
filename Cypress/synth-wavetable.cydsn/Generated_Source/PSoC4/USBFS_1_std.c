...
uint8 USBFS_1_HandleStandardRqst(void) 
{
    ...
    uint32 bmRequestType = USBFS_1_bmRequestTypeReg;
...
void USBFS_1_ConfigReg(void) 
{
    ...
    USBFS_1_ARB_CFG_REG |= (uint32)  USBFS_1_ARB_CFG_CFG_CMP;
    ...
    USBFS_1_SIE_EP_INT_EN_REG = (uint32) USBFS_1_DEFAULT_SIE_EP_INT_EN;
}
    ...
    if (USBFS_1_configuration > 0u)
    {
        ...
        */
        ...
        for (i = 0u; i < ep; i++)
        {
            ...
            {
                curEp  = ((uint32) pEP->addr & USBFS_1_DIR_UNUSED);
                epType = ((uint32) pEP->attributes & USBFS_1_EP_TYPE_MASK);
            ...
            #if (USBFS_1_EP_MANAGEMENT_DMA_AUTO)
                /* Clear IN data ready. */
                USBFS_1_ARB_EP_BASE.arbEp[curEp].epCfg &= (uint32) ~USBFS_1_ARB_EPX_CFG_IN_DATA_RDY;

                /* Select endpoint number of reconfiguration */
                USBFS_1_DYN_RECONFIG_REG = (uint32) ((curEp - 1u) << USBFS_1_DYN_RECONFIG_EP_SHIFT);
                ...
                if (0u != (pEP->addr & USBFS_1_DIR_IN))
                {
                    ...
                    USBFS_1_EP_TYPE_REG &= (uint32) ~(uint32)((uint32) 0x01u << (curEp - 1u));
                ...
                {
                    /* Set endpoint type: 0 - IN and 1- OUT. */
                    USBFS_1_EP_TYPE_REG |= (uint32) ((uint32) 0x01u << (curEp - 1u));
                ...
                */
                USBFS_1_DYN_RECONFIG_REG &= (uint32) ~USBFS_1_DYN_RECONFIG_ENABLE;
...
uint8 USBFS_1_ClearEndpointHalt(void) 
{
    ...
    if ((ep > USBFS_1_EP0) && (ep < USBFS_1_MAX_EP))
    {
        ...
        USBFS_1_SIE_EP_BASE.sieEp[ep].epCnt0 &= (uint32) ~(uint32)USBFS_1_EPX_CNT_DATA_TOGGLE;
